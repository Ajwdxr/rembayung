...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        // Debug - show correct hash for admin123
        $correctHash = password_hash('admin123', PASSWORD_DEFAULT);
        echo '<pre>Correct hash for admin123: ' . $correctHash . '</pre>';
        
        if ($result['success'] && !empty($result['data'])) {
            $admin = $result['data'][0];
            
            // Debug password verify
            echo '<pre>Verifying: ' . $password . ' against ' . $admin['password_hash'] . '</pre>';
            echo '<pre>Result: ' . (password_verify($password, $admin['password_hash']) ? 'MATCH' : 'NO MATCH') . '</pre>';
